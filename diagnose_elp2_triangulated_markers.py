...
with open(BASE / "stereo_config.yaml", "r") as f:
...
for frame_i in range(100):
    ...
    for mid in sorted(set(dl.keys()) & set(dr.keys())):
        ...
        if not np.isfinite(X).all():
            continue
        center, size, edges = marker_stats(X)
        if center[2] <= 0:
            continue
...
print("ELP2 triangulierte Markerdiagnose")
...
for mid in sorted(samples):
    ...
    print(f"ID{mid}: {status} n={len(sizes)} marker_size_med={smed:.2f}cm mad={smad:.2f}cm minmax={smin:.2f}/{smax:.2f}cm z_med={zmed:.2f}m")

pair_defs = [(2, 20), (3, 30), (4, 40)]
print("")
print("Vertikale Paarabstaende aus Stereo-Triangulation")
for a, b in pair_defs:
    ...
    if vals:
        ...
        print(f"ID{a}->ID{b}: n={len(vals)} dist_med={med:.2f}cm mad={mad:.2f}cm minmax={vmin:.2f}/{vmax:.2f}cm")
    else:
