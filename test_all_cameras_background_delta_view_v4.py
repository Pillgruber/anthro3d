...
OUT = BASE / "output" / "all_cameras_background_delta_v4"
...
def delta_candidate(base, person):
    ...
    pv = person["valid_med"]
    delta = bz - pz
    finite = bv & pv & np.isfinite(delta) & np.isfinite(pz)
    if finite.any():
        z_limit = np.percentile(pz[finite], 55) + 0.25
    else:
        z_limit = MAX_Z_M
    raw_dense = finite & (delta > MIN_DELTA_M) & (delta < 1.60) & (pz <= z_limit)
    yy, xx = np.indices(pv.shape)
    sampled = ((xx % PIXEL_STRIDE) == 0) & ((yy % PIXEL_STRIDE) == 0)
    raw = raw_dense & sampled
    clean = clean_mask(raw_dense) & raw_dense
...
def make_row(rig_name, base, person):
    ...
    metric_mask = clean & np.isfinite(person["z_med"]) & np.isfinite(delta)
    z_med = float(np.median(person["z_med"][metric_mask]) * 100) if metric_mask.any() else 0.0
    d_med = float(np.median(delta[metric_mask]) * 100) if metric_mask.any() else 0.0
    row = np.hstack([
...
def dashboard(rows, pending_snapshot_at):
    ...
    cv2.putText(header, "ANTHRO3D BACKGROUND DELTA V4   Raw | Depth | Difference | Candidate   SPACE=Snapshot   q=Ende", (15, 31), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (0, 255, 0), 2)
...
def main():
    print("")
    print("ANTHRO3D Background Delta V4")
    ...
    auto_path = OUT / f"background_delta_v4_auto_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    cv2.imwrite(str(auto_path), auto)
    print(f"Auto-Snapshot gespeichert: {auto_path}")
    pending_snapshot_at = None
    cv2.namedWindow("ANTHRO3D Background Delta V4 - q beendet", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("ANTHRO3D Background Delta V4 - q beendet", 1800, 950)
    try:
        while True:
            dash = dashboard(rows, pending_snapshot_at)
            if pending_snapshot_at is not None and time.time() >= pending_snapshot_at:
                path = OUT / f"background_delta_v4_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            ...
            cv2.imshow("ANTHRO3D Background Delta V4 - q beendet", preview)
