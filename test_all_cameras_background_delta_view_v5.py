...
OUT = BASE / "output" / "all_cameras_background_delta_v5"
...
def delta_candidate(base, person):
    ...
    raw_dense = finite & (delta > MIN_DELTA_M) & (delta < 1.60) & (pz > 0.90) & (pz < 3.20)
...
def dashboard(rows, pending_snapshot_at):
    ...
    cv2.putText(header, "ANTHRO3D BACKGROUND DELTA V5   Raw | Depth | Difference | Candidate   SPACE=Snapshot   q=Ende", (15, 31), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (0, 255, 0), 2)
...
def main():
    print("")
    print("ANTHRO3D Background Delta V5")
    ...
    auto_path = OUT / f"background_delta_v5_auto_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    cv2.imwrite(str(auto_path), auto)
    print(f"Auto-Snapshot gespeichert: {auto_path}")
    pending_snapshot_at = None
    cv2.namedWindow("ANTHRO3D Background Delta V5 - q beendet", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("ANTHRO3D Background Delta V5 - q beendet", 1800, 950)
    try:
        while True:
            dash = dashboard(rows, pending_snapshot_at)
            if pending_snapshot_at is not None and time.time() >= pending_snapshot_at:
                path = OUT / f"background_delta_v5_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            ...
            cv2.imshow("ANTHRO3D Background Delta V5 - q beendet", preview)
