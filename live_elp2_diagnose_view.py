...
OUT = BASE / "output" / "elp2_live_diagnose"
...
def build_dashboard(left, right, depth, mask, count, z_med):
    l = cv2.resize(left, (640, 360))
    r = cv2.resize(right, (640, 360))
    l = label(l, "ELP2 LEFT")
    r = label(r, "ELP2 RIGHT")
    raw = np.vstack([l, r])
    ...
    cv2.putText(dash, f"ELP2 LIVE  points={count}  z={z_med:.1f}cm   SPACE=Snapshot   q=Ende", (15, 32), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (0, 255, 0), 2)
    return dash
...
def main():
    ...
    stereo = make_stereo()
    cv2.namedWindow("ANTHRO3D ELP2 Diagnose Live - q beendet", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("ANTHRO3D ELP2 Diagnose Live - q beendet", 1600, 600)
    ...
    try:
        while True:
            ...
            dash = build_dashboard(rect_l, rect_r, depth, mask, count, z_med)
            if pending_snapshot_at is not None:
                ...
                if time.time() >= pending_snapshot_at:
                    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                    path = OUT / f"elp2_diagnose_{ts}.png"
                    cv2.imwrite(str(path), dash)
                    print("")
                    print(f"Screenshot gespeichert: {path}")
                    pending_snapshot_at = None
            cv2.imshow("ANTHRO3D ELP2 Diagnose Live - q beendet", dash)
            print(f"ELP2 live_points={count} z_median_cm={z_med:.1f}", end="\r")
