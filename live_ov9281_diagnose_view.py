...
OUT = BASE / "output" / "ov9281_live_diagnose"
...
def get_indices():
    ...
    for cam in cfg.get("cameras", {}).get("tracking", []):
        if cam.get("enabled") and cam.get("name") == "OV9281 L":
            idx_l = int(cam.get("device_index"))
        if cam.get("enabled") and cam.get("name") == "OV9281 R":
            idx_r = int(cam.get("device_index"))
    if idx_l is None or idx_r is None:
        raise RuntimeError("OV9281 L/R nicht in config.yaml gefunden")
    return idx_l, idx_r
...
def build_dashboard(left, right, depth, mask, count, z_med, pending_snapshot_at):
    ...
    l = label(l, "OV9281 LEFT")
    r = label(r, "OV9281 RIGHT")
    raw = np.vstack([l, r])
    ...
    cv2.putText(dash, f"OV9281 LIVE  points={count}  z={z_med:.1f}cm   SPACE=Snapshot   q=Ende", (15, 32), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (0, 255, 0), 2)
...
def main():
    idx_l, idx_r = get_indices()
    print(f"OV9281 L Index {idx_l}")
    print(f"OV9281 R Index {idx_r}")

    cap_l = cv2.VideoCapture(idx_l)
    cap_r = cv2.VideoCapture(idx_r)

    cap_l.set(cv2.CAP_PROP_FRAME_WIDTH, FRAME_W)
    cap_l.set(cv2.CAP_PROP_FRAME_HEIGHT, FRAME_H)
    cap_r.set(cv2.CAP_PROP_FRAME_WIDTH, FRAME_W)
    cap_r.set(cv2.CAP_PROP_FRAME_HEIGHT, FRAME_H)

    if not cap_l.isOpened():
        raise RuntimeError(f"OV9281 L Index {idx_l} konnte nicht geoeffnet werden")
    if not cap_r.isOpened():
        raise RuntimeError(f"OV9281 R Index {idx_r} konnte nicht geoeffnet werden")
    ...
    if not ok_l or not ok_r or left is None or right is None:
        cap_l.release()
        cap_r.release()
        raise RuntimeError("Kein OV9281 Frame")

    maps = make_maps((left.shape[1], left.shape[0]))
    stereo = make_stereo()
    pending_snapshot_at = None

    cv2.namedWindow("ANTHRO3D OV9281 Diagnose Live - q beendet", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("ANTHRO3D OV9281 Diagnose Live - q beendet", 1600, 600)
    ...
    try:
        while True:
            ...
            if pending_snapshot_at is not None and time.time() >= pending_snapshot_at:
                ...
                path = OUT / f"ov9281_diagnose_{ts}.png"
                cv2.imwrite(str(path), dash)
                print("")
                print(f"Screenshot gespeichert: {path}")
                pending_snapshot_at = None

            cv2.imshow("ANTHRO3D OV9281 Diagnose Live - q beendet", dash)
            print(f"OV9281 live_points={count} z_median_cm={z_med:.1f}", end="\r")
