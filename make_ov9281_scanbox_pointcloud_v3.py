...
def main():
    print("")
    print("ANTHRO3D OV9281 Scanbox-Punktwolke ohne Personenmaske mit 3D-Clusterfilter")
    print("Ausgabe:", OUT)
    print("")
    roles = camera_roles()
    ...
    if labels.size > 0 and labels.max() >= 0:
        ...
        print(f"Groesster 3D-Cluster: label={best_label} punkte={len(pcd.points)}")
    else:
        print("WARNUNG: Kein stabiler 3D-Cluster gefunden, verwende bereinigte Punktwolke")
    clean_path = OUT / "ov9281_scanbox_cluster_clean.ply"
